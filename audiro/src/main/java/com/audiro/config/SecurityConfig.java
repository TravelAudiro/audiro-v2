...
public class SecurityConfig {
	...
	// 만약 비밀번호를 암호화하지 않으면, HTTP 404(access denied, 접근 거부) 또는
	// HTTP 500 (internal server error, 내부 서부 오류) 에러가 발생함.
	...
	// 스프링 부트 애플리케이션에서 스피링 시큐리티를 이용한 로그인과 로그아웃을 하려면,
	...
	*/
	...
	SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		// 시큐리티 관련 설정들을 구성.
		// CSRF(Close Site Request Forgery) 기능을 비활성화: 
		// CSRF 기능을 활성화한 경우에는,
		...
		http.authorizeHttpRequests((auth)->
			...
			// -> USER 권한이 없는 user3 아이디는 localhost 액세스가 거부된다.
			...
			auth.requestMatchers("/post/create","/post/details","/post/modify","/post/delete","/post/update","/api/comment/**")
				.hasRole("USER")
				.anyRequest() // 적은 경로 외의 나머지 페이지는 모두 허용
				.permitAll() // 허용한다
		);
		*/
